...
export class Bone
{
    ...
    update(pose: Pose): void
    {
        ...
        const x = this.boneToRotationSpace.clone();
        x.premultiply(pose.getJointRotation(this.name));
        x.premultiply(this.rotationToBoneSpace);
        this.transform.rotation.premultiply(x);
